...
HOST_CAMERA_CAPABILITY_LOG="${LOG_DIR}/host-camera-capability-$(date '+%Y%m%d-%H%M%S').log"
HOST_CAMERA_PID=""
...
handle_stereo_capability_failure() {
  ...
  log_host_camera_failure_diagnostics "${HOST_CAMERA_CAPABILITY_LOG}" "${HOST_CAMERA_CAPABILITY_OUTPUT}" "Host stereo capability probe failure"

  if (( driver_exited )) && recover_gemini2_after_host_camera_failure "host stereo capability probe" "${HOST_CAMERA_DEVICE_STATE_BEFORE_LAUNCH}"; then
    recovery_succeeded=1
    current_state="$(gemini2_device_state)"
  fi

  if (( driver_exited )) && [[ "${current_state}" == "usb_present_no_video" ]]; then
    die "The host stereo capability probe failed and the Gemini2 device lost its /dev/video nodes. Automatic recovery did not restore the camera. This NVBlox demo requires stereo IR + depth input and currently supports stereo-capable Orbbec models such as Gemini 330 series, Gemini 2 XL, and Gemini 2 VL."
  fi

  if (( recovery_succeeded )); then
    die "The host stereo capability probe recovered the device, but the current camera or connection still does not expose the stereo IR topics required by this NVBlox + Visual SLAM demo. Use a stereo-capable Orbbec model such as Gemini 330 series, Gemini 2 XL, or Gemini 2 VL."
  fi

  die "The current Orbbec camera or connection does not expose the stereo IR + depth topics required by this NVBlox + Visual SLAM demo. Use a stereo-capable Orbbec model such as Gemini 330 series, Gemini 2 XL, or Gemini 2 VL."
}
...
info "Host $(host_camera_stream_summary) streams are ready for mobile mapping."

if ! probe_container_camera_visibility; then
  die "Host camera streams are ready, but the container cannot discover the expected host camera topics. Check the ROS discovery environment shown above."
fi
