...
IMAGE_NAME="${QWEN36_IMAGE_NAME:-chenduola6/llama-jetson:latest}"
MODEL_REPO="${QWEN36_MODEL_REPO:-unsloth/Qwen3.6-35B-A3B-GGUF}"
MODEL_FILE_NAME="${QWEN36_MODEL_FILE:-Qwen3.6-35B-A3B-UD-Q6_K.gguf}"
MODELS_DIR="${QWEN36_MODELS_DIR:-$HOME/models}"
MODEL_FILE="$MODELS_DIR/$MODEL_FILE_NAME"
HOST_PORT="${QWEN36_PORT:-8080}"
CONTAINER_PORT=8080
CTX_SIZE="${QWEN36_CTX_SIZE:-8192}"
STARTUP_TIMEOUT="${QWEN36_STARTUP_TIMEOUT:-900}"
GPU_FLAGS=()
LIB_MOUNTS=()
...
ensure_docker_access() {
    ...
    case "$choice" in
        y|Y)
            ...
            echo "reComputer run qwen3.6-35b"
            exit 1
            ;;
        *)
...
ensure_image() {
    ...
    if ! "${DOCKER_CMD[@]}" pull "$IMAGE_NAME"; then
        echo "${RED}Failed to pull image $IMAGE_NAME.${RESET}"
        exit 1
    fi
}
...
GPU_LAYERS="${QWEN36_GPU_LAYERS:-$(select_gpu_layers)}"
...
echo ""
echo "Qwen3.6-35B server is ready at: http://127.0.0.1:${HOST_PORT}"
echo ""
echo "Check models:"
...
echo '  -d '\''{"model":"qwen3.6-35b-a3b","messages":[{"role":"user","content":"Hello!"}]}'\'''
echo ""
